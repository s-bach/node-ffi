...
function Library (libfile, funcs, lib) {
  ...
  Object.keys(funcs || {}).forEach(function (func) {
    ...
    var resultType = info[0]
      ...
      , setLastError = fopts && fopts.setLastError

    if (varargs) {
      lib[func] = VariadicForeignFunction(fptr, resultType, paramTypes, abi, setLastError)
    } else {
      var ff = ForeignFunction(fptr, resultType, paramTypes, abi, setLastError)
